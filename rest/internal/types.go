...
type RESTOptions struct {
	ServerIDs []string `json:"serverIds" yaml:"serverIds"`
...
func (ro *RESTOptions) FromValue(value any) error {
	...
	rawServerIds, ok := valueMap["serverIds"]
	if ok && !utils.IsNil(rawServerIds) {
		serverIDs, ok := rawServerIds.([]string)
		if !ok {
			return fmt.Errorf("invalid serverIds in rest options; expected []string, got %v", rawServerIds)
		}
		ro.ServerIDs = serverIDs
	}
